...
The pipeline is summarised in Fig. 2b. Rather than regress $x$, we sample it. A base predictor supplies $x_0$, either through-plane upsampling of $y$ or a frozen restoration backbone. We work in the scaled residual
...
Mean preservation gives an estimator of $\delta$ that needs no 1 mm reference (Fig. 2c). Because $A_w$ preserves slab means, any displacement of $\hat{x}$ appears undiminished in $A_w\hat{x} - y$ over the eroded lung set $\Omega_e$, so
